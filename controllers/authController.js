...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    secure: true, //só receber via htps
  };
  if (process.env.NODE_ENV === 'production') cookieOptions.secure = true; //browser n consegue modificar

  //Devolve um cookie na resposta
  res.cookie('jwt', token, cookieOptions);

  //Limpa a senha pra não voltar na response
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) {
    return next(
      new AppError('The user from this token is no longer active', 401)
    );
  }
